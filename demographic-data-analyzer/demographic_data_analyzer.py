...
def calculate_demographic_data(print_data=True):
    ...
    #print(df.set_index('race').value_counts())
    #print('OOOH \n', df.groupby(['sex'] == 'Male'))
    #print((df['education'].value_counts(normalize=True) * 100).round(1).loc[['Bachelors', 'Masters','Doctorate']])
    #print((df.loc[df['salary'] != '<=50K','education'].value_counts))
    ...
    country=df[['salary','native-country']]
    country_list = df['native-country']
    country_rich_list=country[country['salary'] == '>50K'].loc[:,'native-country']
    # What country has the highest percentage of people that earn >50K?
    highest_earning_country = (country_rich_list.value_counts() / country_list.value_counts() * 100).idxmax()

    highest_earning_country_percentage = round((country_rich_list.value_counts() / country_list.value_counts() * 100).max(),1)

    # Identify the most popular occupation for those who earn >50K in India.
    india= df[(df['salary'] == '>50K') & (df['native-country'] == 'India')].loc[:,'occupation']
